...
func ToProtoBinary(jsonSpec []byte) ([]byte, error) {
	var jsonMap map[string]interface{}
	if err := json.Unmarshal(jsonSpec, &jsonMap); err != nil {
		return nil, err
	}

	doc, err := openapi_v2.NewDocument(jsonToYAML(jsonMap), compiler.NewContext("$root", nil))
	if err != nil {
		return nil, err
	}
